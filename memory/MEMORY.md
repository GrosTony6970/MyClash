...
- **Programme planner** (`apps/api/src/modules/programme/`, admin schedule Programme tab): Auto-suggest may create non-competition blocks (`admin`, `break`, `workshop`) with `matchDurationMinutes=0`; only competition blocks require a tournament, phase, lice count, and positive match duration. Generate schedule should persist the visible programme first, then run the scheduler from saved `event_programme_blocks`.
