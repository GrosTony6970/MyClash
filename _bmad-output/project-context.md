...
- **HTTP validation → Zod via `nestjs-zod`** (migration in progress). The global `ZodOrClassValidationPipe` (`apps/api/src/common/zod-or-class-validation.pipe.ts`) routes `createZodDto` DTOs to Zod and everything else to the legacy `class-validator` pipe, so both coexist. New API inputs: define `z.object({...}).strict()` (`.strict()` preserves `forbidNonWhitelisted`) and `export class XDto extends createZodDto(schema)`. **Don't add new `class-validator` DTOs** — convert the DTO you touch. `main.ts` runs `cleanupOpenApiDoc()` so Zod DTOs render in OpenAPI (keeps `pnpm gen:api-client` isomorphic). ~45 legacy DTOs remain. (Zod is also the rulesets/config validator.)
