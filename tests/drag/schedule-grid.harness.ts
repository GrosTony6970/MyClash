 ...
 * log the assertions read, and the synthetic drag gestures.
 *
 * Split out when the spec crossed the 400-line file cap. The cases belong
 * together — they are the only thing in the repo that mounts ScheduleGrid, and
 * splitting THEM would fragment the net rather than shorten a file.
 *
 * Root `tests/` sits outside typecheck and lint. Only `format:check` and the run
 * itself will tell you something here is broken.
 */

export const ADMIN = 'http://localhost:3003';
export const SCHEDULE_URL = `${ADMIN}/org/${ORG_SLUG}/events/${EVENT_ID}/schedule`;
...
export function card(page: Page, roundCode: string) {
 ...
 */
export async function slotOfCard(page: Page, roundCode: string, liceId: string): Promise<number> {
 ...
 */
export async function dragCardToCell(
  page: Page,
  roundCode: string,
  liceId: string,
  slot: number,
 ...
 */
export async function dragAfterAbandonedDrag(
  page: Page,
