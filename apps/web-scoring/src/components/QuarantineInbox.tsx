...
export function QuarantineInbox({
  ...
  async function handleRetry(entry: RejectedEntry) {
    if (entry.id === undefined) return;
    setBusyId(entry.id);
    try {
      await syncEngine.retryRejectedEntry(entry.id);
      await load();
    } finally {
      setBusyId(null);
    }
  ...
  async function handleDiscard(entry: RejectedEntry) {
    ...
    setBusyId(entry.id);
    try {
      await syncEngine.discardRejectedEntry(entry.id);
      await load();
    } finally {
      setBusyId(null);
    }
          ...
          <ul className="flex flex-col gap-3" data-testid="quarantine-list">
            {entries.map((entry) => (
              ...
              >
                ...
                {/* The server's own words. A 400 carries a real message; only
                    5xx is scrubbed, and a scrubbed one would say so. */}
