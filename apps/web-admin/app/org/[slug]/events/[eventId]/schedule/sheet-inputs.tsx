...
import { LENGTH_FIELDS, withTournamentLength, type LengthField } from './sheet-rows';
...
const SPACING_BOXES: ReadonlyArray<[RequiredField, BoxKind]> = [
  ['matchGapSeconds', 'count'],
  ['minRestMinutes', 'count'],
];
...
/** One Tournament's four boxes. A blank box means "use the Event's length". */
    ...
    <>
      ...
      {LENGTH_FIELDS.map((field) => (
        <input
          ...
          min={1}
          step={1}
          aria-label={t('organizer.schedulePage.planner.config.perTournament.inputAria', {
          ...
          onChange={(e) => {
            ...
            const rows = withTournamentLength(config.tournaments, tournament.id, field, minutes);
            props.onEdit({ ...config, tournaments: rows }, sheetValid(e.currentTarget));
          }}
          className={BOX_CLASS}
        />
      ))}
