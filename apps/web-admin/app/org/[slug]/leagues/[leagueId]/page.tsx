...
export default function OrgLeagueManagePage() {
  ...
  const loadLeague = useCallback(async () => {
    ...
    // endpoint asserts org admin/owner).
  ...
  if (!league) {
    return (
      <main className="mx-auto w-full max-w-5xl px-6 py-12 lg:px-8">
        <Link href={`/org/${orgSlug}/leagues`} className="text-sm text-muted hover:underline">
  ...
  return (
    <main className="mx-auto w-full max-w-5xl px-6 py-12 lg:px-8">
      ...
      <div className="mb-2 text-sm">
        <Link href={`/org/${orgSlug}/leagues`} className="text-muted hover:underline">
          {t('organizer.leagues.manage.backToList')}
        </Link>
      </div>
