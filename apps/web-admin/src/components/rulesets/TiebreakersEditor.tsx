    ...
    <div className="rounded-md border border-border bg-surface p-4">
      {value.length === 0 ? (
        systemFallback && systemFallback.length > 0 ? (
          <ol className="mb-3 space-y-2">
            {systemFallback.map((rule, idx) => (
              ...
              >
                ...
                <span className="font-medium text-foreground">{formatRankingKey(rule.key)}</span>
          ...
          <p className="text-sm italic text-muted">{t('admin.rulesets.tiebreakersEmpty')}</p>
        )
      ) : (
        <ol className="mb-3 space-y-2">
          {value.map((tb, idx) => (
            ...
            >
              ...
              <div className="ml-auto flex items-center gap-1">
                <button
                  ...
                  className="rounded border border-border px-2 py-1 text-xs text-foreground-secondary hover:bg-background disabled:opacity-40"
                ...
                <button
                  ...
                  onClick={() => remove(idx)}
                  disabled={disabled}
                  className="rounded border border-danger/30 bg-danger/10 px-2 py-1 text-xs font-semibold text-danger hover:bg-danger/20 disabled:opacity-50"
