...
describe('PayloadCell', () => {
  ...
  it('falls back to the raw value when no label resolved', () => {
    const html = renderToStaticMarkup(<PayloadCell payload={{ orgId: UUID }} labels={{}} />);
  ...
  it('clamps a long value in the cell but keeps it whole in the details tree', () => {
    ...
    const html = renderToStaticMarkup(
      <PayloadCell payload={{ reason: long, nested: { a: 1 } }} labels={{}} />,
    );
  ...
  it('renders an em dash for an empty or non-object payload', () => {
    expect(renderToStaticMarkup(<PayloadCell payload={null} labels={{}} />)).toContain('—');
    expect(renderToStaticMarkup(<PayloadCell payload={{}} labels={{}} />)).toContain('—');
  });
});
