...
export class ReviewQueueService {
  ...
  private async fetchDeletions(statusFilter: string): Promise<ReviewQueueItem[]> {
    ...
    const userIds = [...new Set(rows.map((r) => r['requester_user_id'] as string))];
    ...
    return rows.map((r) => {
      ...
      return {
        ...
        requesterName:
          userMap.get(r['requester_user_id'] as string) ?? (r['requester_user_id'] as string),
  ...
  private async fetchExchangeEdits(statusFilter: string): Promise<ReviewQueueItem[]> {
    ...
    const userIds = [...new Set(rows.map((r) => r['requested_by_user_id'] as string))];
    const userMap = await this.resolveUserNames(userIds);
    ...
    return rows.map((r) => ({
      ...
      targetHref: null,
      requesterUserId: r['requested_by_user_id'] as string,
      requesterName:
        userMap.get(r['requested_by_user_id'] as string) ?? (r['requested_by_user_id'] as string),
      organizationId: null,
  ...
  private async fetchRulesetSubmissions(statusFilter: string): Promise<ReviewQueueItem[]> {
    ...
    return rows.map((r) => {
      ...
      return {
        ...
        requesterName: submitterId ? (userMap.get(submitterId) ?? submitterId) : 'unknown',
        organizationId: null,
        organizationName: null,
  ...
  private async resolveUserNames(userIds: string[]): Promise<Map<string, string>> {
    ...
    for (const row of (data ?? []) as Array<Record<string, unknown>>) {
      ...
      if (!map.has(uid)) {
        map.set(uid, `${row['given_name'] as string} ${row['family_name'] as string}`);
      }
    }
    return map;
  }
