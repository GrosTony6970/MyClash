...
describe('buildRoundCode against singleElimBracket output', () => {
  it('16 fighters: R16 M1..M8, QF M1..M4, SF M1..M2, F M1, bronze F M2', () => {
  ...
  it('32 fighters: every round starts at M1, never M2', () => {
    ...
    expect(firstCodeByRound.get(1)).toBe('LSW-R32-M1');
    expect(firstCodeByRound.get(2)).toBe('LSW-R16-M1');
    expect(firstCodeByRound.get(3)).toBe('LSW-QF-M1');
    expect(firstCodeByRound.get(4)).toBe('LSW-SF-M1');
    expect(firstCodeByRound.get(5)).toBe('LSW-F-M1');
  });
});
