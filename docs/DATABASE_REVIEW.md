...
- `pnpm db:rls-probe` reads that replay as the anonymous visitor, after seeding `packages/db/fixtures/rls-probe-seed.sql` (a platform role, a draft Event, a draft Tournament under a published Event, a private League, a club's own ruleset). It fails on any read error ("stack depth limit exceeded" was every anonymous read before 0201), on a private seeded row anon can see or a public one it cannot, and on a `public` view without `security_invoker`. CI runs both in the `Database replay` job on Postgres 17. Signed-in reads are not probed: their helper loop is latent (ruling 111a).
