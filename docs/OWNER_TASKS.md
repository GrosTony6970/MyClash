...
- **Critical for GDPR**:
  ...
  - The AI can build the *technical* mechanisms; you write the *legal* text.
...
- **Setup**:
  ...
  - **Restore drill**: Once before beta, restore to a fresh VM and verify integrity. *If you don't test restore, you don't have backups.*
...
- **Decision tree if something breaks**:
  ...
  4. Backend down → all tablets continue offline, sync when back. *(this is exactly what offline-first is for)*
