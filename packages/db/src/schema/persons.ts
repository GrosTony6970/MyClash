...
export const follows = pgTable('follows', {
  id:                    uuid('id').primaryKey().defaultRandom(),
  eventId:               uuid('event_id').notNull().references(() => events.id, { onDelete: 'cascade' }),
  followedPersonId:      uuid('followed_person_id').notNull().references(() => persons.id, { onDelete: 'cascade' }),
  followerUserId:        uuid('follower_user_id'),
  followerGuestSessionId: uuid('follower_guest_session_id').references(() => guestSessions.id, { onDelete: 'cascade' }),
