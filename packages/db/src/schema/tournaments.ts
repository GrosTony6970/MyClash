...
export const bracketSlots = pgTable('bracket_slots', {
  ...
  registrationAId:   uuid('registration_a_id').references(() => registrations.id, { onDelete: 'set null' }),
  registrationBId:   uuid('registration_b_id').references(() => registrations.id, { onDelete: 'set null' }),
});
...
export const poolAssignmentSettings = pgTable('pool_assignment_settings', {
  ...
  enforceFighterRefereeNoOverlap:  boolean('enforce_fighter_referee_no_overlap').notNull().default(true),
