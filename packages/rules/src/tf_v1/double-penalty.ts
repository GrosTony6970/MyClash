/**
 * The double-penalty term, and the whitelist it selects from.
 *
 * CLAUDE.md hard rule 5: no `eval`, no `Function()`. Two forms are accepted,
 ...
 *
 * Only the SCHEMA stayed in @myclash/rulesets. Validating an authored spec is
 * resolution and needs zod; selecting a whitelisted key and running an AST is
 * application and needs nothing.
 */
import { evaluateFormula, type FormulaScope } from '../formula/evaluator';
import { FORMULA_VARIABLE_KEYS, type FormulaNode, type VariableKey } from '../formula/types';
