...
| **Lint**                  | `CI / Lint`                                              | `pnpm turbo run lint` **plus twenty-three further independent gates and three builds** — see [Before pushing](#before-pushing) for the full list. Each runs as its own step with `if: '!cancelled()'`, so one failure never hides the rest. |
...
twenty-six further steps, and shared packages must be built first or a local pass proves nothing
